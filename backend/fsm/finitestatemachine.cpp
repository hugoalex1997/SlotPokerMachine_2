...
namespace backend {
	...
	void FiniteStateMachineImplementation::AddTransition(std::string trigger, std::vector<std::string> from,
														 std::string to) {
		this->AddTransition(Transition(trigger, from, to));
	}

	void FiniteStateMachineImplementation::AddState(State state) { states.emplace_back(state); }

	void FiniteStateMachineImplementation::AddState(std::string id) { this->AddState(State(id)); }
	...
	bool FiniteStateMachineImplementation::TriggerTransition(std::string trigger) {
		...
		for (auto& _from : _fromVec) {
			if (_from == currentState.getId()) {
				...
				std::cout << "Trigger '" << _transition.getTrigger() << "' from '" << previousState.getId() << "' to '"
